...
Before running the PowerShell block, the agent must ask the user to name one known property that identifies B's newer save, such as recent progress, an inventory item, or character position. Record only the user's description in the conversation, never any path, hash, or raw save data. Then tell the user: **the save-selection window will show the verified versions. Select “Sync destination latest”, confirm “Launch with this data”, and do not select a history or bookmark version. The tool will then open DPYes/Grim Dawn. When the game opens, first verify the named B-save property on A. Do not make or save a new change until that property is visibly confirmed. Tell the agent whether it is present. If it is present, make one short, unmistakable A-side change, save normally, and exit the game normally. If it is absent, make no deliberate change, do not save, and exit normally. Do not start another game instance.** Wait for the user to name the property and acknowledge this instruction. The CLI below is the only permitted game launch method.
...
Run this block only after the required user acknowledgement. It suppresses CLI text but displays the interactive save-selection window, then emits one final sentinel. It requires a complete, clear process scan with no active lock or recovery before launch. The expected starting state may be either equal, or `remote_changed_or_unknown` because B advanced the remote while A still records its older applied commit. A differing save is never auto-approved by JSON automation. After launch the block additionally requires that the remote commit changed and the Vault is equal again.
...
try {
    if (-not (Test-Path -LiteralPath $source -PathType Container)) { throw 'source_pull_failed' }
    $checkout = (& git -C $source rev-parse --show-toplevel 2>$null).Trim()
    if ($LASTEXITCODE -ne 0 -or [IO.Path]::GetFullPath($checkout) -ne [IO.Path]::GetFullPath($source)) { throw 'source_pull_failed' }
    ...
    $stage = 'launch_failed'
    $launchOutput = @(& $python -m grim_dawn_sync --config $config launch 2>$null)
    $launchExitCode = $LASTEXITCODE
    if ($launchExitCode -ne 0) { throw 'launch_failed' }
    ...
    if ($after.readiness -ne 'ready' -or
        ...
        $after.remote_commit -eq $before.remote_commit -or $after.last_pushed_commit -ne $after.remote_commit) {
    ...
    $code = if ($_.Exception.Message -match '^(source_pull_failed|existing_environment_missing|existing_config_invalid|existing_config_mismatch|pre_status_failed|preflight_not_ready|preflight_remote_state_inconsistent|launch_failed|post_status_failed|postflight_verification_failed)$') {
        $_.Exception.Message
    } else {
        $stage
    }
