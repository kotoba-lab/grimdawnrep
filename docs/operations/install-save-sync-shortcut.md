...
enrollment. It creates `Grim Dawn (DPYes + Save
Sync).lnk` once and never replaces, deletes, or edits it or the legacy `Grim
Dawn (DPYes).lnk` shortcut.
...
try {
  ...
  $legacy=Join-Path $desktop 'Grim Dawn (DPYes).lnk';$legacyBefore=Fingerprint $legacy
  & $python -m grim_dawn_sync --config $config --json install-shortcut *> $null;if($LASTEXITCODE -ne 0){throw 'shortcut_dry_run_failed'}
  & $python -m grim_dawn_sync --config $config --json install-shortcut --apply *> $null;if($LASTEXITCODE -ne 0){throw 'shortcut_apply_failed'}
  $created=Join-Path $desktop 'Grim Dawn (DPYes + Save Sync).lnk';if(-not(Test-Path -LiteralPath $created -PathType Leaf)){throw 'shortcut_missing'}
  ...
  if((Fingerprint $legacy) -ne $legacyBefore){throw 'legacy_shortcut_changed'}
...
It first verifies the configured machine ID is nonempty/safe, and retains the
legacy shortcut's hash, length, and timestamp exactly. Expected success:
